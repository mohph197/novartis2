...
def split_train_test(df: pd.DataFrame, test_size=0.2, stratify=None, random_state=42) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    # Unique pairs
...
def split_k_fold(df: pd.DataFrame, k_folds=5, random_state=42):
    # Unique pairs
    pairs = df[["country", "brand_name"]].drop_duplicates()
...
def split_bucket_train_test(df: pd.DataFrame, test_size=0.2, stratify_buckets=True, random_state=42) -> tuple[pd.DataFrame, pd.DataFrame]:
    if stratify_buckets:
        stratify = df['label']
    else:
        stratify = None
...
def split_buckets(df: pd.DataFrame, df_aux: pd.DataFrame):
    df = df.merge(
        df_aux[['country', 'brand_name', 'bucket']],
        on=['country', 'brand_name'],
        how='left',
        validate='many_to_one'
    )
    df_b1 = df[df['bucket'] == 1].drop(columns=['bucket'])
    df_b2 = df[df['bucket'] == 2].drop(columns=['bucket'])
